...
export function AuthModal({ isOpen, onClose, initialMode = 'signup' }: AuthModalProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Keep loading indicator active while browser redirects to Google OAuth
    } catch (err: unknown) {
    ...
    >
      <div className="flex flex-col gap-5 pt-3 sm:pt-5 pb-1">
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted mt-1 max-w-xs mx-auto">
            {isLogin
              ...
              : 'Satu tautan untuk semua produk dan sosmed. Siap terima pesanan dalam sekejap.'}
          </p>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full h-11 px-4 rounded-full border border-hairline bg-surface-card hover:bg-surface-soft text-ink text-xs font-medium flex items-center justify-center gap-2.5 transition-all shadow-2xs group"
        >
